...
def compute_centers(num_cluster, x, psedo_labels):# x (N,D)
    n_samples = x.size(0)
    if len(psedo_labels.size()) > 1:
        weight = psedo_labels.T
    else:
        weight = torch.zeros(num_cluster, n_samples).to(x)
...
def compute_cluster_loss(num_cluster,
                         q_centers,#(L,D)
                         k_centers,#(L,D)
                         temperature,
                         psedo_labels):
...
def kl_divergence(similarity_matrix_p, similarity_matrix_q):
    similarity_matrix_p = similarity_matrix_p.float()
    similarity_matrix_q = similarity_matrix_q.float()
    log_p = F.log_softmax(similarity_matrix_p, dim=1)
    q = F.softmax(similarity_matrix_q, dim=1)

    kl = F.kl_div(log_p, q, reduction='batchmean')

    return kl
    
def predict_matrix_transform(label_p ,num_classes):
    one_hot_matrix = np.eye(num_classes, dtype=int)
    one_hot_encoded = one_hot_matrix[label_p]
    result = np.dot(one_hot_encoded,one_hot_encoded.T)
    return result